...
from src.etl import fetch_data, validate_data, transform_data
...
class TestFetchValidateEventContract:
    ...
    def test_fetch_output_event_is_valid_validate_input(
        ...
        per_game_stats = [create_basketball_reference_player_stats(f"Player {i}", "PG", 25, "LAL", 60, 15.0, 5.0, 4.0) for i in range(350)]
        mock_pergame_df = pd.DataFrame(per_game_stats)

        # Advanced stats with complete columns
        advanced_stats = [create_basketball_reference_advanced_stats(f"Player {i}", "PG", 25, "LAL", 60, 15.0, 2.0) for i in range(350)]
    ...
    def test_partition_path_used_consistently(
        ...
        """
        # Mock complete Basketball Reference formatted data
        per_game_stats = [create_basketball_reference_player_stats(f"P{i}", "PG", 25, "LAL", 60, 15.0, 5.0, 4.0) for i in range(350)]
        mock_pergame_df = pd.DataFrame(per_game_stats)
        advanced_stats = [create_basketball_reference_advanced_stats(f"P{i}", "PG", 25, "LAL", 60, 15.0, 2.0) for i in range(350)]
        mock_advanced_df = pd.DataFrame(advanced_stats)
        mock_read_html.side_effect = [[mock_pergame_df], [mock_advanced_df]]
...
class TestEndToEndDataFlow:
    ...
    def test_stats_only_pipeline_end_to_end(
        ...
        per_game_stats = [
            create_basketball_reference_player_stats("LeBron James", "SF", 40, "LAL", 50, 25.0, 7.5, 8.2)
        ] + [
            create_basketball_reference_player_stats(f"Player {i}", "PG", 25, "LAL", 60, 15.0, 5.0, 4.0)
            for i in range(349)
        ]
        mock_pergame_df = pd.DataFrame(per_game_stats)
        advanced_stats = [
            create_basketball_reference_advanced_stats("LeBron James", "SF", 40, "LAL", 50, 24.5, 4.0)
        ] + [
            create_basketball_reference_advanced_stats(f"Player {i}", "PG", 25, "LAL", 60, 15.0, 2.0)
            for i in range(349)
        ]
        mock_advanced_df = pd.DataFrame(advanced_stats)
        mock_read_html.side_effect = [[mock_pergame_df], [mock_advanced_df]]
...
class TestCompletePipelineFlow:
    ...
    def test_stats_only_complete_pipeline(
        ...
        per_game_stats = [
            create_basketball_reference_player_stats("LeBron James", "SF", 40, "LAL", 50, 25.0, 7.5, 8.0),
            create_basketball_reference_player_stats("Anthony Davis", "C", 31, "LAL", 55, 27.0, 12.0, 3.5)
        ] + [
            create_basketball_reference_player_stats(f"Player {i}", "PG", 25, "LAL", 60, 15.0, 5.0, 4.0)
            for i in range(348)
        ]
        ...
        advanced_stats = [
            create_basketball_reference_advanced_stats("LeBron James", "SF", 40, "LAL", 50, 24.5, 4.0),
            create_basketball_reference_advanced_stats("Anthony Davis", "C", 31, "LAL", 55, 26.0, 5.0)
        ] + [
            create_basketball_reference_advanced_stats(f"Player {i}", "PG", 25, "LAL", 60, 15.0, 2.0)
            for i in range(348)
        ]
